    ...
    return (
        <>
            ...
            <div className=' shadow-md w-full flex gap-2 justify-between items-center '>
                ...
                <span className=' md:hidden flex pr-4' onClick={() => { setOpen(state => !state) }}><GiHamburgerMenu size={26} /></span>
                <div
                ...
                {isHovered && (
                    ...
                    >
                                                <span onClick={() => { setIsHovered(false) }} className=' flex w-full text-red-500  cursor-pointer transition-all duration-500 ease-in-out justify-end'><RxCross2 /></span>
                ...
                {clicked && (
                    <div className={`  top-[60px] left-[35%] h-fit overflow-y-scroll rounded-md bg-white shadow-lg p-6 ${searchedCourse.length > 0 ? "flex absolute z-10" : "hidden"} flex-col gap-4`}>
                        <span onClick={() => { setClicked(false); setSearchedCourse([]) }} className=' flex w-full text-red-500  cursor-pointer transition-all duration-500 ease-in-out justify-end'><RxCross2 /></span>
